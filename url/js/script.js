...
function gen_short_url(){
	...
	if(url.length == 0 || url.length <= 5){
		...
		return;
	}else if($('#origin_url').val().substr(0,7) == 'http://' || $('#origin_url').val().substr(0,8) == 'https://'){
		$.ajax({
			type: "POST",
			url: "url.jsp",
			dataType: "json",
			data: "url=" + $('#origin_url').val() ,
			success: function(json){
				...
				if(status_code ==0){
					...
					$("#target_info_title").text(json[0].title);
					$("#target_info_desc").text(json[0].description);
					$('#target_thumb_pic').attr('src', 'https://api.thumbalizr.com/?url=' + encodeURIComponent(json[0].url) + '&api_key=&width=180&quality=80&encoding=jpg');
				}
			},
		...
		show_error_message('請以http://或https://開頭');
		return;
	}
